...
package (`io.davidsusanto.steps`), and Cucumber loads every glue class -- step definitions and hook classes (`@Before`,
...
package, so `Hooks.java` goes there. If I'd put it in `io.davidsusanto.config` or `io.davidsusanto.utils`, those aren't
